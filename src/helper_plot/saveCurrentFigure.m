...
%SCurrentFigure Saves current figure to my figures remote location
%relatively. Saves is .fig (to preserve ALL data embedded in this figure)
%.svg (compatibility) .pdf (for latex documents). Output name is as good as
%unique (time dependant, e.g. Output_2016_12_3_17_51_45.*)
%
%Kevin Denis - KU Leuven - 2016/17

outfilename='D:\Dropbox\Documents\ShareLaTeX\Dissertation\figures\';
if nargin == 0
    c=fix(clock);
    for ii=1:length(c)
        if ii ~= length(c)
            outfilename=[outfilename,num2str(c(ii)),'_'];  %#ok<*AGROW>
        else
            outfilename=[outfilename,num2str(c(ii))];
        end
     end   
else

...
classdef (InferiorClasses = {?casadi.MX,?casadi.SX}) Function
    ...
    methods
        ...
        function [s,varargout] = size(self,ind)
            if nargin == 1
                if nargout <= 1
                    s = self.coeffs.shape;
                else
                    ...
                    for k = 1:nargout-1
                       varargout{k} =  self.coeffs.shape(k+1);
                    end
                end
                    
            else
                s = self.coeffs.shape(ind);
            end
        ...
        function b = ge(self, other)
            ...
            c = self.coeffs;
            if isa(c.data, 'casadi.MX') || isa(c.data, 'sdpvar') || isa(c.data, 'numeric')
                if ~isvector(c(1))
                    nel = prod(c.siz);
                    b = [];
        ...
        function s = optival(self)
            % Overload double for use with YALMIP variables
            s = self.cl(self.basis, optival(self.coeffs));
        end
    ...
    methods (Static)
        ...
        function s = optivar(basis, shape)
            if isscalar(basis) && ~isa(basis, 'cell')
                basis = {basis};
            end
            % dim = num2cell(dim);
            ...
            coeffs = optivar(shape(1) * lengths(1), shape(2) * prod(lengths(2:end)),'x');
            coeffs = Coefficients(coeffs, lengths, shape);
            s = cl(basis, coeffs);
        end
    end
